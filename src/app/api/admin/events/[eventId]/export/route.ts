...
export async function GET(
  ...
  const csvRows = rows.map(
    (r) =>
      `"${r.name}","${r.email}",${r.partySize},"${r.additionalNames.join(" / ")}","${r.startTime}","${r.endTime}","${new Date(String(r.createdAt).endsWith("Z") ? r.createdAt : r.createdAt + "Z").toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })}"`
  );
